"""HTTP client used by application code to send runs and events."""
...
from agent_skill_compiler.sdk.noop import NoopSkillCompilerClient


class SkillCompilerClient:
    """Thin API client for recording runs from user-owned agents."""
    ...
    def from_env(
        ...
        *,
        timeout: float = 30.0,
        optional: bool = False,
    ) -> "SkillCompilerClient | NoopSkillCompilerClient":
        ...
        if optional:
            return NoopSkillCompilerClient()
        missing = [
            name
            for name, value in (
    ...
    def start_run(self, task_name: str, input_text: str, metadata: dict[str, Any] | None = None) -> RunRecord:
        ...
        response = self._client.post("/api/runs/start", json=payload.model_dump(mode="json"))
    ...
    def record_event(
        ...
        *,
        ...
        response = self._client.post(f"/api/runs/{run_id}/events", json=payload.model_dump(mode="json"))
        response.raise_for_status()
    ...
    def finish_run(
        ...
        *,
        ...
        response = self._client.post(f"/api/runs/{run_id}/finish", json=payload.model_dump(mode="json"))
        response.raise_for_status()
        return RunRecord.model_validate(response.json())
    ...
    def analyze(self) -> AnalysisSummary:
        ...
        response = self._client.post("/api/analyze")
        response.raise_for_status()
        return AnalysisSummary.model_validate(response.json())

    def __enter__(self) -> "SkillCompilerClient":
        return self
